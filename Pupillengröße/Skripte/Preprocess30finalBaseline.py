...
df_list_eye_id0_normalised = []
for i, df in enumerate(df_list_eye_id0):
    # Find the index of the closest annotation to the current dataframe
    annotation_index = np.abs(annotation_timestamps - df.iloc[0]['pupil_timestamp']).argmin()
    ...
    df.loc[(df['pupil_timestamp'] >= baseline_start) & (df['pupil_timestamp'] <= baseline_end), 'label'] = 1
    df.loc[(df['pupil_timestamp'] >= stim_start) & (df['pupil_timestamp'] <= stim_end), 'label'] = 2
    df.loc[(df['pupil_timestamp'] >= after_var_start) & (df['pupil_timestamp'] <= after_var_end), 'label'] = 3
    
    # Add a new column to the dataframe containing its own index number
    df['index'] = i
    
    # Append the preprocessed dataframe to the list
    df_list_eye_id0_normalised.append(df)


# Initialize an empty list to store the mean baselines
mean_baselines = []

# Loop through each dataframe and calculate the mean baseline diameter
for df in df_list_eye_id0_normalised:
    mean_diameter = df.loc[df['label'] == 1, 'diameter'].mean()
    mean_baselines.append(mean_diameter)

# Loop through each dataframe again and subtract the mean baseline diameter from diameter for label 2 and 3 rows
for i, df in enumerate(df_list_eye_id0_normalised):
    diameter_diff=df.loc[df['label'].isin([2, 3]), 'diameter_diff'] = df.loc[df['label'].isin([2, 3]), 'diameter'] - mean_baselines[i]
...
# Loop through each dataframe and calculate the mean baseline diameter
for df in df_list_eye_id0_normalised:
    mean_diameter_3d = df.loc[df['label'] == 1, 'diameter_3d'].mean()
    mean_baselines_3d.append(mean_diameter_3d)


# Loop through each dataframe again and subtract the mean baseline diameter from diameter for label 2 and 3 rows
for i, df in enumerate(df_list_eye_id0_normalised):
    diameter_3d_diff=df.loc[df['label'].isin([2, 3]), 'diameter_3d_diff'] = df.loc[df['label'].isin([2, 3]), 'diameter_3d'] - mean_baselines_3d[i]


    
# Loop through each dataframe again and plot the data
for i, df in enumerate(df_list_eye_id0):
    # Create a figure with two subplots
    fig, axs = plt.subplots(ncols=2, figsize=(10, 5))
    
    # Plot the diameter difference data
    axs[0].plot(df.loc[df['label'].isin([2, 3]), 'pupil_timestamp'], df.loc[df['label'].isin([2, 3]), 'diameter_diff'])
    axs[0].set_xlabel('pupil_timestamp')
    axs[0].set_ylabel('diameter_diff')

    # Plot the diameter_3d difference data
    axs[1].plot(df.loc[df['label'].isin([2, 3]), 'pupil_timestamp'], df.loc[df['label'].isin([2, 3]), 'diameter_3d_diff'])
    axs[1].set_xlabel('pupil_timestamp')
    axs[1].set_ylabel('diameter_3d_diff')

    # Set the title of the figure
    fig.suptitle('Dataframe ' + str(i))
...
df_list_eye_id0_normalised[1].to_csv('/Users/Katharina/Desktop/Beispieldaten/example_dataframe.csv', index=False)


#df_list_eye_id0_normalised[0].to_csv('/Users/Katharina/Desktop/Beispieldaten/example_dataframe.csv', index=False)


#stop - look at the normalisation, rest of the code not adapted
#
#
#
#
